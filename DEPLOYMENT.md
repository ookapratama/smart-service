...
> **One-time note for the single-kecamatan redesign (2026-07-28):** this release drops the old `instansi`/`wilayah` multi-tenant tables in favor of a single-kecamatan schema, and several migration files were edited **in place** (same filenames) rather than added as new ones. A plain `migrate` on a server that already ran the old versions would think them already applied and silently skip the schema changes — so `DeployWebhookController` self-detects this via a one-time marker file and runs `migrate:fresh` (which **wipes existing data** — acceptable at this staging stage) automatically on the very next deploy, then falls back to the normal safe incremental `migrate` forever after. No manual step needed — see `DeployWebhookController::SCHEMA_REBUILD_MARKER` if you want the mechanics.
...
`.github/workflows/deploy.yml` runs on every push to `main`: it runs the Pest test suite first (deploy is blocked if tests fail), then builds the app (Composer + npm) on the GitHub Actions runner, uploads the built result to the server over **FTP**, then triggers a short list of `artisan` commands via an **HTTP webhook** on the app itself. The full pipeline (FTP upload → webhook → migrate/seed/cache/queue-restart) runs unattended on every push — nothing needs to be done by hand after the one-time setup below.
...
`db:seed` runs on **every** deploy (not just the first). This is safe because every seeder in this app uses `updateOrCreate`/`updateOrInsert`/existence checks, so re-running never duplicates rows — it just re-asserts the same baseline data (roles, menus, kelurahan, master data, and `DemoDataSeeder`'s sample tiket/pemohon). Real data added through the app (tickets, pemohon, uploaded files) is untouched — seeders never write to those tables. Note `DemoDataSeeder` runs on every deploy including production, until this app moves past staging/demo — see CLAUDE.md.
...
7. Nothing to do by hand for the single-kecamatan schema redesign (2026-07-28) — the webhook self-detects it needs a full rebuild (see the note at the top of this doc) and runs `migrate:fresh` automatically on its first call, exactly once.
...
Seeded automatically on every deploy (`UserSeeder` + `DemoDataSeeder`), password `password` for all except the warga demo account (passwordless — OTP login, a later phase):

| Email                        | Role        | Scope                                                                 |
| ---------------------------- | ----------- | ---------------------------------------------------------------------- |
| `superadmin@gmail.com`       | Super Admin | Full platform access (bypasses all permission checks)                 |
| `admin@gmail.com`            | Admin       | Admin-level management (user/menu/activity-log + S3 modules)          |
| `petugas.soreang@gmail.com`  | Petugas     | Tiket + Pemohon only                                                  |
| `user@gmail.com`             | User        | Dashboard only                                                        |
| `warga.demo@gmail.com`       | Warga       | No password — public citizen role, linked to a demo `pemohon` by NIK  |

**Change or remove these before real citizen data goes live** — they're seeded for testing/demo purposes with a known weak password. Once this stops being a staging/demo deployment, delete `UserSeeder`'s and `DemoDataSeeder`'s calls from `DatabaseSeeder`, or change passwords directly in the database.

## 5. Known gotcha: asdf-managed PHP/Composer PATH

This account uses **asdf** (`.tool-versions`/`.asdf` in the home directory) to manage PHP/Composer versions. A normal interactive login (including cPanel's Terminal) sources `.bash_profile`, which loads asdf's shims into `PATH` — this matters for the **manual one-time steps** in sections 2 and 4 (`php artisan key:generate`, `storage:link`) and for the **queue cron job** in section 6, both of which run `php`/`artisan` directly. If `php: command not found` shows up in either context, confirm `cat ~/.bash_profile` actually initializes asdf (should source `~/.asdf/asdf.sh` or similar), or explicitly `source ~/.bash_profile` first. This does **not** affect `deploy.yml` itself anymore — the workflow no longer runs any command on the server directly; the webhook route runs inside the already-booted app, which doesn't need shell PATH resolution at all.
