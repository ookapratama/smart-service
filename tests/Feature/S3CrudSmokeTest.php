...
beforeEach(function () {
    ...
    $this->instansi = Instansi::create(['name' => 'Kecamatan Uji', 'level' => 'kecamatan', 'kode' => 'UJI']);
});

test('admin can view instansi index and detail pages', function () {
    $this->actingAs($this->admin)->get(route('instansi.index'))->assertOk();
    $this->actingAs($this->admin)->get(route('instansi.show', $this->instansi->id))->assertOk();
});

test('admin can create an instansi', function () {
    $this->actingAs($this->admin)->post(route('instansi.store'), [
        'name' => 'Desa Uji',
        'level' => 'kelurahan',
        'kode' => 'DUJ',
        'parent_id' => $this->instansi->id,
    ])->assertRedirect(route('instansi.index'));

    $this->assertDatabaseHas('instansi', ['kode' => 'DUJ', 'parent_id' => $this->instansi->id]);
});
...
test('admin can view and create pemohon scoped to an instansi', function () {
    ...
    $this->assertDatabaseHas('pemohon', ['nik' => '3204011234567890', 'instansi_id' => $this->instansi->id]);
});

test('admin can view tiket queue and transition its status with a status_log entry', function () {
    $pemohon = Pemohon::create(['instansi_id' => $this->instansi->id, 'nik' => '3204019999999999', 'name' => 'Warga Tiket']);
    $jenis = JenisSurat::create(['kode' => 'TKT', 'nama' => 'Surat Tiket']);
    $pengajuan = \App\Models\PengajuanSurat::create(['jenis_surat_id' => $jenis->id, 'keperluan' => 'uji tiket']);
...
test('invalid status transition is rejected', function () {
    $pemohon = Pemohon::create(['instansi_id' => $this->instansi->id, 'nik' => '3204018888888888', 'name' => 'Warga Tolak']);
    $jenis = JenisSurat::create(['kode' => 'RJT', 'nama' => 'Surat Reject']);
    $pengajuan = \App\Models\PengajuanSurat::create(['jenis_surat_id' => $jenis->id, 'keperluan' => 'uji reject']);
